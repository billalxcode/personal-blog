...
This design document outlines the changes required to migrate the personal blog domain from `www.masbill.xyz` to `masbill.xyz`.
...
- Replace main url `https://masbill.xyz` with `https://masbill.xyz`.
- Replace the Agentic AI Framework project website link `https://agentic-ai.masbill.xyz` with `https://masbill.xyz`.
...
- Replace `www.masbill.xyz` with `masbill.xyz` (or `www.masbill.xyz` if representing the layout) to match the new canonical domain.
...
- Replace `masbill.xyz` in the Open Graph footer description with `masbill.xyz`.
